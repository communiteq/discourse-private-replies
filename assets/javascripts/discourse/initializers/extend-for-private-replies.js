...
function registerTopicFooterButtons(api, container, siteSettings) {
  api.registerTopicFooterButton({
    ...
    displayed() {
      ...
      if (
        siteSettings.private_replies_on_selected_categories_only === false ||
        topic?.category?.custom_fields?.private_replies_enabled
      ) {
        return (
          this.currentUser &&
          (this.currentUser.id === topic_owner_id || this.currentUser.staff)
        );
      }
      return false;
    },
